    ...
    try {
        ...
        if (!email || !phone) {
            return res.status(400).json({ 
                success: false, 
                error: 'email dan phone wajib diisi' 
            });
        }
        ...
        res.json({ 
            ...
            data: { 
                ...
                message: "Kode OTP 4-6 digit telah dikirim via SMS" 
            } 
        });
    } catch (e) {
        res.status(400).json({ 
...
app.get('/gopay/token', async (req, res) => {
    try {
        ...
        if (!otp || !otp_token) {
            return res.status(400).json({ 
                success: false, 
                error: 'otp dan otp_token wajib diisi' 
            });
        }
        ...
        if (!/^\d{4,6}$/.test(otp)) {
            return res.status(400).json({ 
                success: false, 
                error: 'OTP harus 4-6 digit angka' 
            });
        }
...
app.get('/gopay/payouts', async (req, res) => {
    try {
        ...
        const data = await goMerchant.getPayouts(token);
        res.json({ success: true, data });
    } catch (e) {
        res.status(400).json({ 
            success: false, 
...
// QRIS Dinamis → menghasilkan gambar & upload ke hosting
...
app.get('/gopay/qris/status', async (req, res) => {
    try {
        ...
        if (!token || !amount || !created_at) {
            return res.status(400).json({ 
                success: false, 
                error: 'token, amount, dan created_at wajib diisi' 
            });
        }
...
if (require.main === module) {
    app.listen(PORT, () => {
        console.log(`GoPay Merchant API running on port ${PORT}`);
    });
}
